  ...
  if (!WaterOnly)
    {
      ...
      ss << "RootFiles_SelectedEvents/Simus_" << particleType << "_BWater"
	 << "_Threshold" << setprecision(3) << armorDetectionThreshold << "keV_EnergyDeposited_" << prefix << "_" << selectedVolumes << ".root";
    
    }
  else if (WaterOnly)
    {
      
      ss << "RootFiles_SelectedEvents/Simus_" << particleType << "_Water"
	 << "_Threshold" << setprecision(3) << armorDetectionThreshold << "keV_EnergyDeposited_" << prefix << "_" << selectedVolumes << ".root";
...
void SelectEvents(string partType, bool Armor = 0, bool Water = 0, bool Concrete = 0, bool Iron = 0, bool IronOnTop = 0)
{
  double DetectionThreshold = 0.;
  int BWaterThickness = 400;
  bool WaterOnly = 1 ;
  string SelectedVolumes = "or";
  ...
  // const TString& fName = "/AGWeber/girardcarillo/ShieldingSimus_ID.root";
  
  string fName = "/AGWeber/girardcarillo/" + partType + "/ShieldingSimus_ID";

  if (IronOnTop) {

    if (Armor && Water && Concrete && Iron) {
      fName += "_OV_Water_Concrete_Iron" ;
    }
    else if (!Armor && Water && Concrete && Iron) {
      fName += "_Water_Concrete_Iron" ;
    }
    else if (!Armor && !Water && Concrete && Iron) {
      fName += "_Concrete_Iron" ;
    }
    else if (!Armor && !Water && !Concrete && Iron) {
      fName += "_Iron" ;
    }
    ...
    if (Armor && Water && Iron && Concrete) {
      fName += "_OV_Water_Iron_Concrete" ;
    }
    else if (!Armor && Water && Iron && Concrete) {
      fName += "_Water_Iron_Concrete" ;
    }
    else if (!Armor && !Water && Iron && Concrete) {
      fName += "_Iron_Concrete" ;
    }
    else if (!Armor && !Water && !Iron && Concrete) {
      fName += "_Concrete" ;
    }
  ...
  //     if (Concrete && Iron) {
  // 	if (IronOnTop) {
  // 	  fName = fName + "_Concrete_Iron" ;
  // 	}
  // 	else
  // 	  fName = fName + "_Iron_Concrete" ;
  //     }
  //     else if (!Concrete && Iron) {
  // 	fName = fName + "_Iron" ;
  //     }
  //     else if (Concrete && !Iron) {
  // 	fName = fName + "_Concrete" ;
  //     }
  //   }
  // }
  ...
  cout << "\n" << "Executing Root Macro SelectEvents with file " << fName << "\n\n";
  ...
  int ActiveVolumeID = geant4Metadata->GetActiveVolumeID("Armor");
  ...
  // int Total_number_ev_loop = 10; // for tests
  int Total_number_ev_loop = run->GetEntries(); // total number of events in the run
  if (Total_number_ev_loop > 10000)
    Total_number_ev_loop = 10000;
  ...
  for (size_t i = 0; i < Total_number_ev_loop; i++)
      ...
      else
	{
	  ...
	  cout << "-\"or\" for selecting Sensitive OR Armor events," << "\n";
	  cout << "-\"and\" for selecting Sensitive AND Armor events," << "\n";
	...
	int ArmorSensitiveTransportationMultiplicity = 0;
	...
	for (int trackIndex=0; trackIndex<event->GetNumberOfTracks(); trackIndex++) 
	  // for (int trackIndex=0; trackIndex<5; trackIndex++)
	  {
		...
		if (aTrack.GetProcessName(aHitCollection.GetHitProcess(hitIndex)) == "Transportation"
		    && geant4Metadata->GetGeant4GeometryInfo().GetVolumeFromID(aHitCollection.GetHitVolume(hitIndex)) == "Armor"
		    && geant4Metadata->GetGeant4GeometryInfo().GetVolumeFromID(aHitCollection.GetHitVolume(hitIndex + 1)) == "Sensitive")
		  {

		    ArmorSensitiveTransportationMultiplicity++;
		  ...
		  } // end if condition on transportation Armor -> Sensitive
	...
	if (ArmorSensitiveTransportationMultiplicity <= 1)
	  {
	...
	// If there is more than one hit
	else
	  {
	    ...
	    ArmorSensitiveTransportationMultiplicity = trackHitVector.size();
	    
	  } // end of else if condition if more than one hit

	// cout << "with multiplicity " << ArmorSensitiveTransportationMultiplicity << endl;
	
	h_multiplicity->Fill(ArmorSensitiveTransportationMultiplicity);
	
	if (ArmorSensitiveTransportationMultiplicity)
	  {
	...
	// cout << "Multiplicity " << ArmorSensitiveTransportationMultiplicity << endl;
	...
	// Fill 1D histogram to store energy deposited in Armor
	...
	for (int i = 0; i < trackHitVector.size(); ++i) {
	  ...
	  // tous les ev devraient etre sur linterface Armor/scintillateur
  ...
  // cout << "\n\nevents interacting before the Armor volume " << counter << " on total events " << Total_number_ev_loop << endl;

  // Correcting the total number of events from events not depositing energy in any volume (events interacting before the Armor volume)
  ...
  *Thickness_vs_AttenuationAllPart << Attenuation << " " << StdAttenuation << "\n";
  ...
  *Thickness_vs_AttenuationNeutron << AttenuationNeutron << " " << StdAttenuationNeutron  << " " << "\n"; // same comment here, AttenuationNeutron looks like miscalculated
  ...
  *Thickness_vs_AttenuationGamma << AttenuationGamma << " " << StdAttenuationGamma  << " " << "\n"; // same comment here, AttenuationGamma looks like miscalculated
  ...
  *Thickness_vs_AttenuationGenerator << AttenuationGenerator << " " << StdAttenuationGenerator  << " " << "\n";
  ...
  string OutputRootFileName_Armor = generateOutputFileName(PrimaryParticleType.Data(), PrimaryEnergy, BWaterThickness, WaterOnly, DetectionThreshold, SelectedVolumes.c_str(), "Armor");
